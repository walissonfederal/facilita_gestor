...
if($acao == 'load'){
    ...
    $read_financeiro_paginator = ReadComposta("SELECT financeiro_id, financeiro_valor, financeiro_status, financeiro_valor_pagamento, financeiro_data_vencimento FROM financeiro WHERE financeiro_tipo = '".$_GET['OP']."' {$_SESSION['financeiro_load']}");
    ...
    $read_financeiro = Read('financeiro', "WHERE financeiro_tipo = '".$_GET['OP']."' {$_SESSION['financeiro_load']} LIMIT $inicio,$maximo");
    ...
    if(NumQuery($read_financeiro) > '0'){
        foreach($read_financeiro as $read_financeiro_view){
            ...
            $tabela .= '<tr>';
                $tabela .= '<td>'.$read_financeiro_view['financeiro_codigo'].'</td>';
                $tabela .= '<td>'.$tipo_financeiro.'</td>';
                $tabela .= '<td>'.$read_financeiro_view['financeiro_descricao'].'</td>';
                $tabela .= '<td>'.GetDados('contato', $read_financeiro_view['financeiro_id_contato'], 'contato_id', 'contato_nome_fantasia').'</td>';
                $tabela .= '<td>'.GetDados('contato', $read_financeiro_view['financeiro_id_contato'], 'contato_id', 'contato_telefone').'</td>';
                $tabela .= '<td>'.GetDados('contato', $read_financeiro_view['financeiro_id_contato'], 'contato_id', 'contato_email').'</td>';
                $tabela .= '<td>'.GetDados('tipo_documento', $read_financeiro_view['financeiro_id_tipo_documento'], 'tipo_documento_id', 'tipo_documento_descricao').'</td>';
                $tabela .= '<td>'.$status_financeiro.'</td>';
                $tabela .= '<td>'.FormDataBr($read_financeiro_view['financeiro_data_vencimento']).'</td>';
                $tabela .= '<td>'.FormatMoney($read_financeiro_view['financeiro_valor']).'</td>';
                $tabela .= '<td>'.$data_pagamento_financeiro.'</td>';
                $tabela .= '<td>'.$valor_pagamento_financeiro.'</td>';
                $tabela .= '<td>' . FormatMoney($ValorMulta) . '</td>';
                $tabela .= '<td>' . FormatMoney($ValorJuros) . '</td>';
                $tabela .= '<td>' . FormatMoney($ValorAtualizado) . '</td>';
            $tabela .= '</tr>';
        }
    }
