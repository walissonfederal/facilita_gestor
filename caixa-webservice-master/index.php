...
require_once '_api/ConsultaBoleto.class.php';
require_once '_api/AlteraBoleto.class.php';
require_once '_api/RegistraBoleto.class.php';
$read_financeiro = Read('financeiro', "WHERE financeiro_id IN(".$getIdRemessa.") AND financeiro_status = '0'");
...
foreach($read_financeiro as $read_financeiro_view);
	...
	if(NumQuery($read_contato) > '0'):
		...
		$quantidade_caracter = strlen($read_contato_view['contato_cpf_cnpj']);
		...
		$url_operacao = 'https://barramento.caixa.gov.br/sibar/ManutencaoCobrancaBancaria/Boleto/Externo';
		...
		$dados_retorno_consulta = array(
			'urlIntegracao' => $url_consulta,
			'codigoCedente' => '273007',
			'nossoNumero' => $_SESSION['NOSSO_NUMERO'],
			'dataVencimento' => $_SESSION['DATA_VENCIMENTO'],
			'valorNominal' => $_SESSION['VALOR'],
		...
		$dados_retorno_operacao = array(
			'urlIntegracao' => $url_operacao,
			'codigoCedente' => '273007',
			'nossoNumero' => $_SESSION['NOSSO_NUMERO'],
			'dataVencimento' => $_SESSION['DATA_VENCIMENTO'],
			'valorNominal' => $_SESSION['VALOR'],
		...
		$integracao_consulta = new ConsultaBoleto($dados_retorno_consulta);
		$return_consulta = $integracao_consulta->consultaRegistro();

		if($return_consulta['CONTROLE_NEGOCIAL']['COD_RETORNO'] == '1' && $return_consulta['CONTROLE_NEGOCIAL']['MENSAGENS']['RETORNO'] == '(47) NOSSO NUMERO NAO CADASTRADO PARA O BENEFICIARIO'):
					
			$integracao_operacao = new RegistraBoleto($dados_retorno_operacao);
			$return_operacao = $integracao_operacao->realizarRegistro();
			
			if($return_operacao['CONTROLE_NEGOCIAL']['COD_RETORNO'] == '0' && $return_operacao['CONTROLE_NEGOCIAL']['MENSAGENS']['RETORNO'] == '(0) OPERACAO EFETUADA'):
				//$link_boleto = $return_operacao['INCLUI_BOLETO']['URL'];
			endif;
			
			echo '<pre>';
			print_r($return_operacao);
		elseif($return_consulta['CONTROLE_NEGOCIAL']['COD_RETORNO'] == '0' && $return_consulta['CONTROLE_NEGOCIAL']['MENSAGENS']['RETORNO'] == '(0) OPERACAO EFETUADA - SITUACAO DO TITULO = EM ABERTO'):
			if($return_consulta['CONSULTA_BOLETO']['TITULO']['DATA_VENCIMENTO'] != $_SESSION['DATA_VENCIMENTO'] || $return_consulta['CONSULTA_BOLETO']['TITULO']['VALOR'] != $_SESSION['VALOR']):
				$integracao_altera = new AlteraBoleto($dados_retorno_altera);
				$return_altera = $integracao_altera->alteraRegistro();
				
				if($return_altera['CONTROLE_NEGOCIAL']['COD_RETORNO'] == '0' && $return_altera['CONTROLE_NEGOCIAL']['MENSAGENS']['RETORNO'] == '(0) OPERACAO EFETUADA'):
					//$link_boleto = $return_altera['ALTERA_BOLETO']['URL'];
				endif;
				echo '<pre>';
				print_r($return_altera);
			else:
				//$link_boleto = $return_consulta['CONSULTA_BOLETO']['TITULO']['URL'];
				echo '<pre>';
				print_r($return_consulta);
			endif;		
		else:
			echo '<pre>';
			print_r($return_consulta);
