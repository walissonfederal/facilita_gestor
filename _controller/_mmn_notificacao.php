...
if($acao == 'load'){
    ...
    $read_notificacao_paginator = ReadComposta("SELECT notificacao_id FROM notificacao WHERE notificacao_id_user = '".$_SESSION['PROJETO_CDL']['user_id']."'");
    $read_notificacao = Read('notificacao', "WHERE notificacao_id_user = '".$_SESSION['PROJETO_CDL']['user_id']."' ".$order_by." LIMIT $inicio,$maximo");
    ...
    if(NumQuery($read_notificacao) > '0'){
        ...
        echo '<a href="#" onclick="carrega_pagina(\'mmn_ticket\', \'update.php?id='.$read_notificacao_view['notificacao_id_ticket'].'\');">Abrir Ticket</a>';
