...
function Create($Tabela, array $Dados) {
    ...
    if ($QuerySqlCreate) {
        QueryAuditoria(date('Y-m-d'), date('Y-m-d H:i:s'), $Tabela, 'create', $descricao_query, $_SESSION[VSESSION]['user_id']);
        return true;
    } else {
        echo $SqlCreate;
    }
...
function Update($Tabela, array $Dados, $Condicao = NULL) {
    ...
    if ($QueryUpdate) {
        QueryAuditoria(date('Y-m-d'), date('Y-m-d H:i:s'), $Tabela, 'update', $descricao_query, $_SESSION[VSESSION]['user_id']);
        return true;
    } else {
        echo $SqlUpdate;
    }
...
function Delete($Tabela, $Condicao = NULL) {
    ...
    if ($QueryDelete) {
        QueryAuditoria(date('Y-m-d'), date('Y-m-d H:i:s'), $Tabela, 'delete', $Condicao, $_SESSION[VSESSION]['user_id']);
        return true;
    } else {
