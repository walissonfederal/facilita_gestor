<?php
	...
	$dados_refresh = $inicio_dados_correto + 50;
	//echo '<meta http-equiv="refresh" content="2;url=teste_sms.php?inicio_dados='.$dados_refresh.'">';
	
	$referencia = '05/2020';
	
	$read_chip = Read('chip_app_copy3', "WHERE chip_count_sms > '0' ORDER BY chip_id ASC LIMIT $inicio_dados_correto,50");
	
	/*if(NumQuery($read_chip) > '0'):
		foreach($read_chip as $read_chip_view):
			
			$read_quantidade = Read('quantidade', "WHERE quantidade_id_contato = '".$read_chip_view['id_contato']."' AND quantidade_referencia = '".$referencia."'");
			if(NumQuery($read_quantidade) > '0'):
				foreach($read_quantidade as $read_quantidade_view):
					$update_quantidade['quantidade_sms'] = $read_quantidade_view['quantidade_sms'] + $read_chip_view['chip_count_sms'];
					Update('quantidade', $update_quantidade, "WHERE quantidade_id = '".$read_quantidade_view['quantidade_id']."'");
				endforeach;
			else:
				$create_quantidade['quantidade_id_contato'] = $read_chip_view['id_contato'];
				$create_quantidade['quantidade_sms'] = $read_chip_view['chip_count_sms'];
				$create_quantidade['quantidade_referencia'] = $referencia;
				Create('quantidade', $create_quantidade);
			endif;
		endforeach;
	endif;
